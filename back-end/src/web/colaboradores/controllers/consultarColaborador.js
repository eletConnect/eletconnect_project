...
exports.consultarColaborador = async (request, response) => {
    ...
    try {
        const { data: colaboradorData, error: colaboradorError } = await supabase
            .from('usuarios')
            .select('matricula, nome, cargo, email, status, avatar, instituicao')
            .eq('matricula', matricula)
            .eq('instituicao', instituicao); // Filtra também pela instituição

        if (colaboradorError) {
            return response.status(500).json({ mensagem: 'AErro ao consultar o colaborador', detalhe: colaboradorError.message });
        }

        if (!colaboradorData || colaboradorData.length === 0) { // Verificação adicional para garantir que há dados
