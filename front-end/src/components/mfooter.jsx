...
const MenuItem = ({ path, label, icon, className }) => (
    <li className={`text-center ${className || ''}`}>
        <Link to={path} className='d-flex flex-column btn'>
            <i className={icon}></i>
...
export default function MFooter() {
    ...
    const itensMenu = [
        { path: '/m/home', label: 'Home', icon: 'bi bi-house-fill fs-1' },
        { path: '/m/my-electives', label: 'Minhas eletivas', icon: 'bi bi-bookmark-star-fill fs-1' },
        { path: '/m/electives', label: 'Eletivas', icon: 'bi bi-journal-bookmark-fill fs-1' },
