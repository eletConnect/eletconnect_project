...
export default function MHome() {
    ...
    const handleFiltroChange = useCallback((filtro, setFiltro) => {
        setFiltro(prev => prev.includes(filtro) ? prev.filter(f => f !== filtro) : [...prev, filtro]);
    }, []);
    ...
    const eletivasFiltradas = useMemo(() => eletivas
        .filter(eletiva => eletiva.tipo === abaSelecionada)
        .filter(eletiva => filtroDia.length === 0 || filtroDia.includes(eletiva.dia))
        .filter(eletiva => filtroHorario.length === 0 || filtroHorario.includes(eletiva.horario)), [eletivas, abaSelecionada, filtroDia, filtroHorario]);
    ...
    return (
        <>
            <MHeader />
            <main id="mMain" className="d-flex flex-column align-items-center">
                {carregando ? (
                    ...
                    <>
                        ...
                        <div className='d-flex flex-column btn-group-sm gap-2 m-2'>
                            <div className="btn-group btn-group-sm" role="group" aria-label="Basic checkbox toggle button group">
                                <input type="checkbox" className="btn-check" id="btncheck1" autoComplete="off" onChange={() => handleFiltroChange('Terça-feira', setFiltroDia)} />
                                <label className="btn btn-outline-secondary" htmlFor="btncheck1">Terça-feira</label>

                                <input type="checkbox" className="btn-check" id="btncheck2" autoComplete="off" onChange={() => handleFiltroChange('Quinta-feira', setFiltroDia)} />
                                <label className="btn btn-outline-secondary" htmlFor="btncheck2">Quinta-feira</label>
                            </div>
                            <div className="btn-group btn-group-sm" role="group" aria-label="Basic checkbox toggle button group">
                                <input type="checkbox" className="btn-check" id="btncheck3" autoComplete="off" onChange={() => handleFiltroChange('1º e 2º horário', setFiltroHorario)} />
                                <label className="btn btn-outline-secondary" htmlFor="btncheck3">1º e 2º horário</label>

                                <input type="checkbox" className="btn-check" id="btncheck4" autoComplete="off" onChange={() => handleFiltroChange('3º e 4º horário', setFiltroHorario)} />
                                <label className="btn btn-outline-secondary" htmlFor="btncheck4">3º e 4º horário</label>

                                <input type="checkbox" className="btn-check" id="btncheck5" autoComplete="off" onChange={() => handleFiltroChange('5º e 6º horário', setFiltroHorario)} />
