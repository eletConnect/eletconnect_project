...
import EditarAluno from './editarAluno'; // Verifique o caminho para o componente
...
export default function Alunos() {
    ...
    const carregarAlunosComEletivas = async () => {
        setCarregando(true);
        try {
            // Chama o novo endpoint que retorna todas as eletivas de todos os alunos
    ...
    const alternarOrdenacao = (coluna) => {
        setOrdenacao((prevOrdenacao) => ({
            coluna,
            ascendente: prevOrdenacao.coluna === coluna ? !prevOrdenacao.ascendente : true,
        }));
        setPaginaAtual(1);
    };
    ...
    const toggleSelectAluno = (matricula) => {
        setAlunosSelecionados((prevSelecionados) =>
            prevSelecionados.includes(matricula) ? prevSelecionados.filter((item) => item !== matricula) : [...prevSelecionados, matricula]
        );
    };
