...
export default function Verification() {
    ...
    useEffect(() => {
        const checkSessionAndInstitution = async () => {
            try {
                const responseSession = await axios.get('http://localhost:3001/auth/check-session');
                ...
                handleError(error, 'A Redirecionando...');
            }
        };
    ...
    const checkInstituicao = async (userId) => {
        try {
            const responseEscola = await axios.post('http://localhost:3001/instituicao/verificar', { id: userId });
