...
export default function Eletiva() {
    ...
    const excluirEletiva = async (e) => {
        ...
        try {
            const resposta = await axios.post('/eletivas/excluir', { codigo: codigoParaEditar, instituicao: usuario.instituicao, tipo: tipoParaEditar });
            if (resposta.status === 200) {
                carregarEletivas();
    ...
    const handleSalvarPeriodo = async (e) => {
        e.preventDefault();

        try {
            // Cria objetos Date a partir das datas fornecidas
            ...
            const response = await axios.post('/eletivas/definir-periodo', {
                instituicao: usuario.instituicao, // Substitua pelo ID da instituição correto
                dataInicio: dataInicioBrasilia.toISOString(),
                dataFim: dataFimBrasilia.toISOString()
            });

            if (response.status === 200) {
                showToast('success', 'Período de inscrições definido com sucesso!');
            } else {
                        ...
                        {!isLoading && (
                            <div className='d-flex gap-2'>
                                <button className='btn btn-outline-secondary' data-bs-toggle="modal" data-bs-target="#defirnirPeriodo">
                                    <i className="bi bi-calendar2-check-fill me-2"></i>Definir período
                                </button>
                                                    ...
                                                    <tr key={eletiva.codigo}>
                                                        ...
                                                        <td className='text-end'>
                                                            <div className="d-flex align-items-center justify-content-end gap-3">
                                                                ...
                                                                <button className='btn btn-sm btn-danger d-flex align-items-center gap-2' data-bs-toggle="modal" data-bs-target="#excluirEletiva" onClick={() => { setCodigoParaEditar(eletiva.codigo); setNomeParaEditar(eletiva.nome); setTipoParaEditar(eletiva.tipo) }} >
            ...
            <div className="modal fade" id="editarEletiva" tabIndex="-1" aria-labelledby="editarEletivaLabel" aria-hidden="true">
                <div className="modal-dialog modal-xl">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <iframe id='editarEletivaFrame' src={`http://localhost:5173/electives/edit?codigo=${codigoParaEditar}`} width="100%" height="455px"></iframe>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="modal fade" id="defirnirPeriodo" tabIndex="-1" aria-labelledby="definirPeriodoLabel" aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <form onSubmit={handleSalvarPeriodo}>
                                <div className="mb-3">
                                    <label htmlFor="dataFim" className="form-label">Fim do periodo de inscrições das eletivas</label>
