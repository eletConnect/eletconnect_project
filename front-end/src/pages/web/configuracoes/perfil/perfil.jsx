...
export default function SettingsPerfil() {
    ...
    const alterarFoto = async (e) => {
        ...
        if (file) {
            setFoto(file); // Armazena o arquivo da foto no estado
            const previewUrl = URL.createObjectURL(file); // Cria uma URL para visualização da foto
            setFotoUrl(previewUrl); // Atualiza o URL da visualização
        }
    };
