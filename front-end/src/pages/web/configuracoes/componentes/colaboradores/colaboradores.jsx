...
export default function Colaboradores() {
    ...
    const listarColaboradores = async () => {
        try {
            const response = await axios.post('http://localhost:5173/colaborador/listar', { instituicao: escola.id });
            setData(response.data.alunosData);
        } catch (error) {
            showToast('Erro ao listar os alunos', 'error');
        }
    }
    ...
    const cadastrarColaborador = async (e) => {
        ...
        try {
            await axios.post('http://localhost:5173/colaborador/cadastrar', { matricula, nome, email, cargo, instituicao: escola.id });
