...
export default function Instituicao() {
    ...
    const armazenarLogo = async (logo, nome) => {
        ...
        try {
            ...
            console.error('Erro ao armazenar logo', error);
            throw error;
        }
    };
    ...
    const cadastrarInstituicao = async (e) => {
        ...
        try {
            const logoUrl = logo ? await armazenarLogo(logo, nome) : '';
            const { data, error } = await axios.post('http://localhost:3001/instituicao/cadastrar', {
    ...
    const entrarInstituicao = async (e) => {
        ...
        try {
            const { data, error } = await axios.post('http://localhost:3001/instituicao/entrar', { id: user.id, codigo });
            if (error) throw new Error(error.message);
